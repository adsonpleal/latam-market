...
function parsePacketStream(body: Uint8Array, keys: CryptKeys): PacketChunk[] {
  ...
  while (ptr + 10 <= body.byteLength) {
    ...
    // O guard de comprimento já garante os dois bytes; `noUncheckedIndexedAccess`
    // não consegue enxergar isso através do ternário.
...
/** Primeiro container genérico de um tipo, se existir. */
